export const Header = ({ profile, setProfile, onReset }) => {
    ...
    return (
        <header className="fixed top-0 w-full p-6 flex justify-between items-center bg-darkBg/60 backdrop-blur-lg z-50">
            {/* Logo */}
            <div onClick={() => {setProfile('all'); onReset()}} className="text-2xl cursor-pointer font-black tracking-tighter flex transition-all duration-400 hover:scale-107 ">
                Elias<div className="text-accent  transition-all duration-300">Sgv</div>
            </div>
                ...
                <div className="flex p-1 rounded-full border border-white/10 shadow-2xl scale-90 md:scale-100 gap-2">
                    {profile !== 'all' && (
                            <button 
                                ...
                                className={`px-6 py-2 rounded-full text-[10px] cursor-pointer font-bold transition-all duration-300 hover:bg-[rgb(0,255,247)] hover:shadow-[0_0_15px_rgb(0,255,247,.8)] ${
                                    profile === 'all' 
